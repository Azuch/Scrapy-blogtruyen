...
class BspiderSpider(scrapy.Spider):
    ...
    def parse(self, response):
        base = "https://m.blogtruyen.vn"
        chapters_links = response.xpath("//article[@class='list-chapter']//a/@href").extract()
        manga_name = response.url.split("/")[-1]
        chapters_links.reverse()
        for index, chapter_link in enumerate(chapters_links):
            yield scrapy.Request(url=urljoin(base, chapter_link), callback=self.parse_chapter, meta={"manga_name": manga_name, "chapter_name": f"chapter-{index}"})
    ...
    def parse_chapter(self, response):
        ...
        # Extract image URLs
        imgs = response.xpath("//article[@class='chapter-detail']//img/@src").extract()
